...
export default async function PlanVersionPage({ params }: { params: { slug: string; id: string } }) {
  ...
  return (
    <PageShell title={`Plan version ${run.version}`} description="Every optimization and re-plan is kept as its own version.">
      <PlanVersionClient slug={params.slug} runId={run.id} canPlan={canPlan(user.role)} canDispatch={canApproveOverride(user.role)} dayHref={`/t/${params.slug}/dispatch?date=${run.runDate.toISOString().slice(0, 10)}&depot=${run.depotId}`} />
    </PageShell>
  );
}
