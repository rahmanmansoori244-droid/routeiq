...
- A re-plan (late order, **Re-plan**, **Use instead**) keeps the driver of each trip: the same truck and trip first, then the driver of the truck's nearest trip, then the truck's **Default driver**. It never gives a driver two trucks at the same time: a trip that now overlaps one of the driver's locked, loading or dispatched loads, or another of their trips because the re-plan moved it, gets the next driver in that order, or **No driver** for you to fill (of two trips, the one that moved loses the driver). Only when you had already given one driver two planned trips at the same time can that driver stay on both, with the yellow warning. Check the drivers after every re-plan.
