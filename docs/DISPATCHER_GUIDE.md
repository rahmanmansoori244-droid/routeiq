...
- A driver you pick in the **Driver** list stays on that trip (the same truck and trip number) through every re-plan (late order, **Re-plan**, **Use instead**), even when the new times overlap another trip of that driver or one of their locked, loading or dispatched loads: the plan then shows the yellow warning, and you decide who drives which. Under the list, such a driver reads **picked by hand**.
...
- If a re-plan or **Use instead** has no such trip (for example **MIN_TRUCKS** leaves a truck out, or a truck gets fewer trips), the plan says so in yellow, for example *Driver picked by hand, not in this plan: you picked Bob for T03 · L1 (08:00–10:00), and this plan has no such trip*. RouteIQ remembers your pick: when a later re-plan or **Use instead** has that truck and trip again, Bob goes back on it (unless he is no longer active).
- Every other driver RouteIQ fills in: the driver the trip already had, then the driver of the truck's nearest trip, then the truck's **Default driver**. When a re-plan moves one of two trips of the same driver onto the other's hours (or onto their locked, loading or dispatched load), the trip that moved gets the next driver in that order, or **No driver** for you to fill. The one overlap RouteIQ leaves: a trip keeps the driver it had next to a trip where you picked that same driver yourself, with the yellow warning. A late order, **Re-plan** and **Use instead** decide the same trips the same way, whichever truck is listed first.
- A trip whose driver a re-plan or **Use instead** changed is listed in yellow on the plan, for example *Driver changed by this plan: T02 · L1 (10:00–12:00) Ali → Sam, because Ali is on T03 · L1 at that time*, until you pick a driver for that trip (another one, or **Keep** to agree with RouteIQ's); the re-plan's message and **Use instead** also say how many driver notes the plan has. Check the drivers after every re-plan.
- **No driver** is not kept through a re-plan: RouteIQ fills the trip in again (the driver the trip had in the previous plan version, the driver of the truck's nearest trip, or the truck's usual driver) and says so in yellow, for example *Driver added by this plan: T03 · L1 (08:00–10:00) now has Sam (the trip had no driver)*. The driver it fills in is never marked as your pick, and never put next to a trip where you picked that same driver yourself. The same holds when **Use instead** goes to an option without that trip and back. To stop a driver being put back (for example, on leave), clear the truck's **Default driver** or set the driver to inactive under **Drivers**.
