...
- A re-plan (late order, **Re-plan**, **Use instead**) keeps the driver you chose for each trip: the same truck and trip first, then the driver of the truck's nearest trip, then the truck's **Default driver**. RouteIQ never puts a driver on a trip that overlaps one of their locked, loading or dispatched loads, and never guesses one driver onto two trucks at the same time; such a load is left with **No driver** for you to fill. If two planned trips that you gave the same driver now overlap, the driver stays on both and the plan shows the yellow warning. Check the drivers after every re-plan.
...
- If the connection drops (or RouteIQ is being updated) while you click, you see *The server could not be reached*, and the buttons work again at once. Check the connection, reload the page to see whether the change was saved, and click again if it was not.
...
While the new day loads, the screen shows *Loading ...* and its buttons are off; if it cannot be loaded, you see the error and **Try again** instead of the previous day. Everything you do (upload, confirm, optimize) is always for the day on the screen. If you change the date while a lock, OPTIMIZE or a save is still running, it finishes for the day you started it on, and the screen then shows the date you picked.
