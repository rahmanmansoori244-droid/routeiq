...
- A trip that lost or changed its driver is listed in yellow on the plan, for example *Driver changed by this plan: T02 · L1 (10:00–12:00) Ali → Sam, because Ali is on T03 · L1 at that time*, until you set a driver for that trip (another one, **Keep** to agree with RouteIQ's, or **No driver**); after that it is not listed again. A trip that only got a driver (it had none) is not listed. The re-plan's message and **Use instead** also say how many driver notes the plan has. Check the drivers after every re-plan.
