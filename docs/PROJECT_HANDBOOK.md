...
| Yellow warnings | request and solver warnings from the chosen `ScenarioDetails`; the driver notes of `summaryJson.driverChanges` (`driverChangeWarnings` in `driver-links.ts`, fed with `getPlanDetail`'s own loads, whose `driverHandSet` is read from each row's marker): "Driver changed by this plan: ..." for a trip that lost or changed its driver, until the dispatcher sets that trip's driver (another driver, or **Keep**); "Driver picked by hand, not in this plan: ..." for a `TRIP_GONE` note while the plan has no load for that truck and trip |
...
- **Per load.** The driver picker calls `PATCH /api/runs/:id/loads/:loadId { driverId }` (`null` = no driver), which runs `updateLoad` then `setDriverTx`.
  ...
  - The load is marked as the dispatcher's choice: `driverSetById` (the user) and `driverSetAt` (fourth review of PR3, migration `20260926110000_plan_load_driver_set_by`). "No driver" clears both. `createNextVersion`'s copies keep them. A driver is **hand-set** when either column is set (`isHandSetDriver`: deleting the user clears `driverSetById`, `driverSetAt` stays). `getPlanDetail` gives the screen `driverHandSet`, read from each load row. A driver set before that migration has no marker and counts as filled in by RouteIQ until the dispatcher presses **Keep**. No backfill: a `LOAD_DRIVER_SET` row names the load it was written on; a copy-forward copy links back to it (`carriedFromLoadId`), but a load `applyScenario` created does not, so a backfill would mark some of the choices made before the update and not others.
...
- **After every optimize, re-plan or "Use instead".** `applyScenario` calls `planDrivers(trips, evidence, usable)` (`load-state.ts`) before it deletes anything. A **trip** is a truck and load number. The **evidence** is the loads of the version the plan is applied to, as they are right before the apply: for "Use instead", the version's own loads; for the re-plan job, the copies `createNextVersion` made of the previous version's loads (nobody can change them while the job runs). So both read the same kind of evidence, and the same evidence gives the same drivers and notes, whatever order the optimizer lists the trips in. A trip's **evidence load** is the evidence load on the same truck and trip, if any. `usable` = the tenant's active drivers.
  ...
  - **Pass 2, drivers RouteIQ fills in.** Every other trip, the one that moved least first (how far its departure moved from its evidence load's; stable; trips without an evidence load last), gets the first of these that is active and not on an overlapping trip already given out (a frozen load, a hand-set trip, or an earlier trip of pass 2):
  ...
  - **Driver notes.** A note only when a trip loses or changes the driver its evidence load had: `CLASH` (another trip of the plan got that driver at an overlapping time; the note names that trip), `INACTIVE` (that driver is no longer active), and `TRIP_GONE` (a hand-set driver whose truck and trip the new plan does not have). Filling a trip that had no driver is not a note. `applyScenario` stores the notes with the names in `summaryJson.driverChanges`, audits them in `SCENARIO_CHOSEN` and returns them (the "Use instead" toast and the job message count them). The plan lists a note in yellow until the dispatcher sets that trip's driver - any driver, or the same one with **Keep** - and a `TRIP_GONE` note while the plan has no load for that trip ([3.7](#37-step-5-reviewing-the-plan)). A `TRIP_GONE` note is information about this version only: nothing brings that driver back later. A later plan with that truck and trip fills it in like any other trip, and the dispatcher picks the driver again.
...
| `dispatch-load-state.spec.ts` | `lib/dispatch/load-state.ts` | load status graph, load order per truck, frozen/on-road sets, the driver rules of a re-plan or "Use instead" (`planDrivers`: frozen loads, hand-set drivers kept, the trip that moved loses a filled-in driver, the candidate order, trip 1 and 2 of a truck, the notes), `isHandSetDriver`, `isDriverKeep`, `driverPickLink` |
