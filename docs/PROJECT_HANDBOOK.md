...
- **After every optimize, re-plan or "Use instead".** `applyScenario` calls `assignReplanDrivers(newLoads, now, parent, kept, usable)` (`load-state.ts`). For each new load it takes the strongest evidence first:
  ...
  No step, step 1 included, puts a driver on a load that overlaps a load of that driver on another truck: one of their kept (locked, loading or dispatched) loads - a kept load cannot move, so that is always a double booking (`timesClash` against `kept`) - or a load already given out in this plan. Such a load goes to the next step, or stays without a driver. The one exception is in step 1: two trips of this version that already had the same driver at overlapping hours before this plan (the version's own load times, read with the drivers) keep that driver on both - the dispatcher's own clash. Step 1 gives the trips that moved least first, so when a re-time puts one trip of a driver onto the hours of another, the trip that moved loses the driver. Only active drivers of the tenant count. "No driver" is not carried over. Second review of PR3: a PLANNED load the job had just created (not a copy) was step-1 evidence without any check, so "Use instead" re-timing its trip onto the hours of the driver's LOCKED load gave that driver two sheets. Third review of PR3: step 1 was checked only against kept loads, so "Use instead" re-timing one of two trips the job had given the same driver onto the other's hours kept the driver on both.
- **Clashes.** A clash the dispatcher sets by hand is allowed but shown (`driverClashes`, `driverClashNotes`). The one clash a re-plan or "Use instead" can keep: two planned loads of this version that already had the same driver at overlapping hours (step 1, the exception above); it shows the same warning. After a re-plan, the new version's own evidence is only its frozen copies and the copies whose driver was changed on it, so a hand-set clash between two untouched planned loads of the parent is not carried either. A hand-set clash with a kept load is not carried into the next plan: that load gets the next source or no driver.
