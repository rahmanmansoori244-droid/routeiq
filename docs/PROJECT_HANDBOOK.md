...
| L16 | **Late-order part fixed (stabilization PR2):** inactive customer / product answer 409 `CUSTOMER_INACTIVE` / `PRODUCT_INACTIVE` naming it, and the date must be real (`isoDateSchema`); a lock wait past the transaction timeout answers 409 `INTAKE_BUSY`. **The job-debug 404 remains (security PR, PR1).** Was: **Plain `Error`s become 500s.** The late-order route throws `new Error('Customer ... is inactive.')` inside its transaction, and the job-debug route throws `new Error('Not found')` for a job of another run (its comment intends a 404). `handleError` in `lib/api.ts` turns both into "Internal server error" | `app/api/dispatch/late-order/route.ts`, `app/api/runs/[id]/jobs/[jobId]/debug/route.ts` |
