...
- It picks a driver for every new load with `planDrivers` ([3.9](#39-step-7-a-driver-per-load)) before anything is deleted: the evidence is this version's loads as they are then (for the re-plan job, the copies of the previous version's loads). It keeps the driver notes (a trip that lost or changed its driver, a hand-set driver whose trip the plan does not have).
...
- It sets `chosenScenarioId` and the status by the loads: `DISPATCHED` (with `finalizedAt`) when every load of the version is already out, otherwise `READY` (PR3: before, always READY). The write is conditional (never over a superseded row). It then calls `refreshPlanFacts` with the driver notes and writes audit `SCENARIO_CHOSEN` (with `driverChanges`: truck, trip, from, to, reason, when any). It returns `{ driverChanges }`: `chooseScenario` passes them on (choose-scenario answers `driversChanged`, the count the screen's toast names: "N driver note(s)"), and the job's message adds "N driver note(s) (see the plan)" (on `RunJob.message`; the dispatch screens show a job's message only while it runs or after it failed). This happens on every optimize, not only when the dispatcher switches options.

**`refreshPlanFacts(tx, tenantId, runId, { driverChanges? })`** recomputes and stores on `RunPlan`:
...
- `summaryJson` = `computeSummary` (`apps/web/lib/dispatch/summary.ts`): orders served, partial and unserved; cases; service % per P1 to P5; trucks, loads, km, hours, utilisation, fuel, cost; revenue and margin served (null unless every order has a value); late orders; unserved by reason; loads by status; provider and solver info; `driverChanges`, the driver notes of the applied plan (`DriverChangeNote`: truck, trip, times, `from` and `to` with their names, reason `CLASH`, `INACTIVE` or `TRIP_GONE`, and for `CLASH` the trip that got the driver). `applyScenario` passes the new list (the key is left out when it is empty); a refresh after a load change keeps the list already stored. `createNextVersion` copies the summary with the notes; they are never read as evidence, and the next applied plan replaces them. A `parkedDrivers` list written by earlier builds of this PR is ignored, and the next refresh leaves it out.
...
| Yellow warnings | request and solver warnings from the chosen `ScenarioDetails`; the driver notes of `summaryJson.driverChanges` (`driverChangeWarnings` in `driver-links.ts`, fed with `getPlanDetail`'s own load rows and their marker, `driverSetByDispatcher`): "Driver changed by this plan: ..." for a trip that lost or changed its driver, until the dispatcher sets that trip's driver (another driver, **Keep**, or "No driver" on a trip that has a driver; it does not come back after; a trip the plan left without a driver keeps its note until a driver is picked for it, as a re-sent "No driver" changes nothing); "Driver picked by hand, not in this plan: ..." for a `TRIP_GONE` note while the plan has no load for that truck and trip |
...
- Otherwise it runs `applyScenario` on the same version. No new version is created, and `planDrivers` sets the drivers ([3.9](#39-step-7-a-driver-per-load)); the toast says how many driver notes the option left (`driversChanged`).
...
- **Per load.** The driver picker calls `PATCH /api/runs/:id/loads/:loadId { driverId }` (`null` = no driver), which runs `updateLoad` then `setDriverTx`.
  - `checkDriverChange` refuses a real change once the load is DISPATCHED or COMPLETED. Re-sending the current driver changes nothing, with one exception, **Keep** (`isDriverKeep`): the driver RouteIQ filled in (not hand-set) re-sent for a load not yet out is marked as the dispatcher's choice, with a `LOAD_DRIVER_SET` row (`kept: true`). The plan screen's **Keep** link sends it; the Driver list cannot (choosing the option already selected fires no change).
  - The driver must exist and be active. Audit `LOAD_DRIVER_SET`.
  - The load is marked as the dispatcher's choice: `driverSetById` (the user) and `driverSetAt` (fourth review of PR3, migration `20260926110000_plan_load_driver_set_by`). "No driver" chosen for a load that has a driver is marked the same way (who, when), so a driver note on that trip ends for good (`driverSetByDispatcher`; review of the simplified rules: "No driver" used to clear both, and a note that had left the trip without a driver came back after the dispatcher picked a driver and then "No driver"). "No driver" for a load that has none changes nothing (`checkDriverChange`: unchanged; the Driver list does not even send it): a note that left the trip without a driver ends only when a driver is picked for it. `createNextVersion`'s copies keep them. A driver is **hand-set** when the load has a driver and either column is set (`isHandSetDriver`: deleting the user clears `driverSetById`, `driverSetAt` stays); "No driver" is never hand-set, there is no driver to keep. `getPlanDetail` gives the screen `driverHandSet`, read from each load row. A driver set before that migration has no marker and counts as filled in by RouteIQ until the dispatcher presses **Keep**. No backfill: a `LOAD_DRIVER_SET` row names the load it was written on; a copy-forward copy links back to it (`carriedFromLoadId`), but a load `applyScenario` created does not, so a backfill would mark some of the choices made before the update and not others.
...
- **After every optimize, re-plan or "Use instead".** `applyScenario` calls `planDrivers(trips, evidence, usable)` (`load-state.ts`) before it deletes anything. A **trip** is a truck and load number. The **evidence** is the loads of the version the plan is applied to, as they are right before the apply: for "Use instead", the version's own loads; for the re-plan job, the copies `createNextVersion` made of the previous version's loads, with any driver the dispatcher set on a copy between the new version and the start of its job (nobody can change them while the job runs), never the previous version's own loads. So both read the same kind of evidence, and the same evidence, with the trips in the same order, gives the same drivers and notes. The order matters only on a tie in pass 2 (below): the job and "Use instead" both read the optimizer's list, which is sorted by truck code, then trip. A trip's **evidence load** is the evidence load on the same truck and trip, if any. `usable` = the tenant's active drivers.
  - **Pass 0, frozen loads.** Locked, loading, dispatched and completed loads (any status but PLANNED) stay as they are, drivers included; their times take their drivers.
  - **Pass 1, hand-set drivers.** A trip whose evidence load has a hand-set driver keeps that driver and its marker, even when the new times overlap a frozen load or another hand-set trip of that driver: the overlap shows as the yellow clash warning (`driverClashes`). A filled-in trip at those times gives the driver up (pass 2). The marker is copied from the evidence row (its foreign key keeps `driverSetById` valid; it is never read from JSON). A hand-set driver who is no longer active is not kept: the trip goes to pass 2, and an `INACTIVE` note says why.
  - **Pass 2, drivers RouteIQ fills in.** Every other trip, the one that moved least first (how far its departure moved from its evidence load's; trips without an evidence load last; stable: on a tie - the same move, or no evidence load - the trip the optimizer lists first goes first: it lists by truck code, then trip), gets the first of these that is active and not on an overlapping trip already given out (a frozen load, a hand-set trip, or an earlier trip of pass 2):
    1. its evidence load's driver;
    2. the driver of the same truck's trip in the new plan that is nearest in time (the smallest gap between the two trips, the earlier one on a tie) and already has a driver - a frozen load counts, so the driver of a locked Load 1 takes the new Load 2;
    3. the truck's default driver.

    None of them: no driver, for the dispatcher to fill. So of two filled-in trips that now overlap with the same driver, the one that moved loses it, and a driver RouteIQ fills in never overlaps another trip of that driver.
  - **Overlap** is the planned times alone (`timesOverlap`: back at 10:00 and out at 10:00 is no overlap). Trip 1 and trip 2 of a truck share a driver unless their times really overlap, which a valid plan never has.
  - **The first optimization of a day** has no evidence: pass 2 with 2 and 3 only, so each truck's default driver in the optimizer's order, never one driver on two overlapping trips - the same drivers as before these rules.
  - **Driver notes.** A note only when a trip loses or changes the driver its evidence load had: `CLASH` (another trip of the plan got that driver at an overlapping time; the note names that trip), `INACTIVE` (that driver is no longer active), and `TRIP_GONE` (a hand-set driver whose truck and trip the new plan does not have). Filling a trip that had no driver is not a note. `applyScenario` stores the notes with the names in `summaryJson.driverChanges`, audits them in `SCENARIO_CHOSEN` and returns them (the "Use instead" toast and the job's message count them; the dispatch screens show a job's message only while it runs or after it failed, so after a re-plan the dispatcher sees the yellow notes). Every apply replaces the stored notes, also with none: a re-plan's version starts with a copy of the previous version's summary, notes included (shown while the copy is in use), and its job's plan replaces them. A load change keeps them. The plan lists a note in yellow until the dispatcher sets that trip's driver - any driver, the same one with **Keep**, or "No driver" when the trip has a driver - and never again after (the row's marker decides, `driverSetByDispatcher`; a note that left the trip without a driver stays until a driver is picked for it, since a re-sent "No driver" writes no marker), and a `TRIP_GONE` note while the plan has no load for that trip ([3.7](#37-step-5-reviewing-the-plan)). A `TRIP_GONE` note is information about this version only: nothing brings that driver back later. A later plan with that truck and trip fills it in like any other trip, and the dispatcher picks the driver again.
  - These rules replaced a multi-step design after the sixth review of PR3 (owner decision, [7.4](#74-review-leads-found-while-writing-this-handbook)).
- **Clashes.** A clash the dispatcher sets by hand is allowed but shown (`driverClashes`, `driverClashNotes`). A re-plan or "Use instead" keeps it when a hand-set trip overlaps another hand-set trip of that driver or one of their frozen loads: the same warning. A driver RouteIQ fills in never creates a clash.
...
| `SCENARIO_CHOSEN` | RunPlan | `applyScenario` (every optimize and every "Use instead"); `driverChanges` lists the driver notes (truck, trip, from, to, reason: `CLASH`, `INACTIVE` or `TRIP_GONE`) |
...
| `dispatch-load-state.spec.ts` | `lib/dispatch/load-state.ts` | load status graph, load order per truck, frozen/on-road sets, the driver rules of a re-plan or "Use instead" (`planDrivers`: frozen loads, hand-set drivers kept, the trip that moved loses a filled-in driver, a tie in the optimizer's order, the candidate order, trip 1 and 2 of a truck, the notes), `isHandSetDriver`, `driverSetByDispatcher`, `isDriverKeep`, `driverPickLink` |
| `choose-scenario-route.spec.ts` | `app/api/runs/[id]/choose-scenario/route.ts` | the `driversChanged` count (a hand-set driver whose trip the option drops, a changed driver) |
...
**Stabilization release, PR3 "Plan lifecycle and concurrency".** Fixed. Since the fourth review it has one additive migration (`20260926110000_plan_load_driver_set_by`: the hand-set driver marker) and one new optional key in `summaryJson` (`driverChanges`); nothing else changes the schema or a stored JSON shape:
...
- **Review of PR3, fixed before merge** (all confirmed findings):
  ...
  - *Driver double-booking* (medium): the PLANNED copies a re-plan carries forward were taken as the new plan's drivers without a clash check, so a re-timed trip could put one driver on two trucks at once. Every driver RouteIQ fills in is now checked against the trips already given out, frozen loads included ([3.9](#39-step-7-a-driver-per-load)).
...
- **Second review of PR3 (the fix commits), fixed before merge** (all confirmed findings):
  ...
  - *Driver double-booking through "Use instead"* (low, pre-existing): a driver kept from the version's own loads was not checked against its frozen loads; now it is ([3.9](#39-step-7-a-driver-per-load)). Tests: `tests/lib/dispatch-load-state.spec.ts`, `tests/lib/plan-lifecycle.spec.ts`.
...
- **Third review of PR3 (checking the second round of fixes), fixed before merge:**
  ...
  - *"Use instead" and two trips of one driver* (low): re-timing one of two trips the job had given the same driver onto the other's hours kept the driver on both. Now the trip that moved loses the driver ([3.9](#39-step-7-a-driver-per-load)). Tests: `tests/lib/dispatch-load-state.spec.ts`, `tests/lib/plan-lifecycle.spec.ts`.
...
- **Fourth review of PR3 (checking the third round of fixes), fixed before merge:**
  - *Re-plan job: the trip that did not move lost its driver* (medium): the job decided its trips in the optimizer's order (truck code), so when a re-plan moved one trip onto the hours of another trip of the same driver, the trip listed first kept the driver, whether it had moved or not, even over a driver the dispatcher had set by hand, and "Use instead" gave the opposite answer for the same trips. Now the trip that moved least goes first, and a hand-set driver stays on its trip ([3.9](#39-step-7-a-driver-per-load)). Tests: `tests/lib/dispatch-load-state.spec.ts`, `tests/lib/plan-lifecycle.spec.ts` (job apply on a copy-forward version and "Use instead", both truck orders).
  - *"Use instead" replaced a hand-set driver without a word* (medium, from the third round): RouteIQ could not tell a driver the dispatcher chose from one it filled in. `PlanLoad.driverSetById` / `driverSetAt` now record the dispatcher's choice (set by `setDriverTx`, copied forward, carried by `applyScenario` to the same truck and trip); a hand-set driver is kept even when the new times overlap, shown as the yellow warning; every trip that loses or changes its driver is listed on the plan ("Driver changed by this plan"), in `SCENARIO_CHOSEN` and in the "Use instead" toast. Tests: the same, `tests/lib/driver-pack.spec.ts` (the notes), `tests/integration/plan-lifecycle-db.spec.ts` (both cases on real PostgreSQL).
...
- **Fifth review of PR3 (checking the fourth round of fixes), fixed before merge:**
  - *A hand-set driver dropped by an option without its trip* (medium): "Use instead" (or a re-plan) to an option that did not have the hand-set truck and trip - MIN_TRUCKS often leaves a truck out, and a truck can get fewer trips - deleted the choice with no note. It is now a `TRIP_GONE` note on the plan, in `SCENARIO_CHOSEN`, the "Use instead" count and the job message ([3.9](#39-step-7-a-driver-per-load)). Tests: `tests/lib/dispatch-load-state.spec.ts`, `tests/lib/plan-lifecycle.spec.ts`, `tests/lib/choose-scenario-route.spec.ts`, `tests/integration/plan-lifecycle-db.spec.ts`.
  - *"Pick it again" did nothing* (medium and low, two findings): a driver chosen before the update has no marker, and the guide said to pick it again, but the Driver list fires no change for the driver already selected, and the server ignored a re-sent driver. The server now marks a re-sent driver RouteIQ filled in (**Keep**, `isDriverKeep`, audited), the screen shows "picked by hand" or a **Keep** link (`DetailLoad.driverHandSet`), and a driver note ends once the dispatcher sets that trip's driver. No backfill ([3.9](#39-step-7-a-driver-per-load) says why). Tests: the same files, and the jsdom harness outside the repository (Keep sends the driver, then "picked by hand").
  - *Tests that did not catch a revert* (low): the moved-least order, a hand-set driver who is no longer active, the `driversChanged` count and the job message suffix now each have a test that fails when that part is reverted.
- **Sixth review of PR3 (checking the fifth round of fixes), fixed before merge:**
  - *"Use instead" put a cleared driver back and double-booked them* (medium): the dispatcher moved Bob from T3 to T2 (T2 -> Bob, T3 -> No driver); "Use instead" then gave T3 the previous version's Bob, next to the hand-set T2 trip, while a re-plan job from the same screen gave T3 its default driver. Superseded by the simplified rules below: a plan reads only the loads of the version it is applied to, and a driver RouteIQ fills in never overlaps a trip given out.
  - *After "No driver", a round trip through an option without the trip gave back an older hand-set driver* (low). Superseded by the simplified rules below: nothing brings a driver back.
...
- **Simplified driver rules (owner decision after the sixth review), before merge.** Every round of the driver rules found new edge cases; the seventh review confirmed more (among them: a note never ended on the real plan screen after Keep, because `getPlanDetail` passed rows without `driverHandSet`; "Use instead" could still double-book through the previous version's driver; a driver kept in the summary for a dropped trip could come back onto a driver already on the road, or fail an apply once its user was deleted). The rules were replaced by one short rule set ([3.9](#39-step-7-a-driver-per-load)): the evidence is the loads of the version being applied, frozen loads keep their drivers, a hand-set driver stays on its trip, every other trip - the one that moved least first - gets its own driver, the driver of the truck's nearest trip or the truck's default, whichever is active and free first, and a note only when a trip loses or changes its driver. Removed: the separate evidence of the previous version, the drivers kept in the summary for dropped trips (an older summary's list is ignored) and the notes for trips that were only filled in. `getPlanDetail` now feeds the notes its own loads, so a note ends on the screen and in Excel once the dispatcher sets that trip's driver. Tests: `tests/lib/dispatch-load-state.spec.ts` (every pass, the candidate order, moved-least, the notes), `tests/lib/plan-lifecycle.spec.ts` (the re-plan job and "Use instead" give the same drivers and notes for the same evidence, in both truck orders; frozen loads; an inactive and a dropped hand-set driver; Keep ends a note in `getPlanDetail`; an older summary), `tests/lib/choose-scenario-route.spec.ts`, `tests/lib/driver-pack.spec.ts`, `tests/integration/plan-lifecycle-db.spec.ts`; the rules were checked by mutation (the two reviews below found rules no test pinned yet; they are pinned now). `tests/lib/repo-guards.spec.ts` keeps the removed promises (a dropped driver coming back, "Driver added" notes) and helper names out of the docs and `lib/`.
- **Review of the simplified driver rules, fixed before merge** (all confirmed findings, low):
  - *A note came back after "No driver"*: a note that had left a trip without a driver ended when the dispatcher picked a driver, and came back when the dispatcher then chose "No driver" (it cleared the marker). "No driver" is now marked too (who, when; not a hand-set driver), and the notes read that marker (`driverSetByDispatcher`), so a note ends for good once the dispatcher sets that trip's driver ([3.9](#39-step-7-a-driver-per-load)).
  - *The docs promised more than the code*: 3.9 and the guide said the drivers did not depend on the optimizer's trip order. They do on a tie (the same move, or no evidence load): the trip the optimizer lists first goes first, and it lists by truck code. The code follows the stable rule; the docs now say so, and `tests/lib/repo-guards.spec.ts` keeps the old wording out.
  - *Rules without a test that fails when they break*: the re-plan job reads its version's copies (with a driver the dispatcher set on a copy before the job started), not the previous version's loads; a tie keeps the optimizer's order, also on the first optimization of a day; a carried marker names the user who set the driver, not the one applying the plan; `TRIP_GONE` is in the `SCENARIO_CHOSEN` audit; an apply without notes replaces the stored ones (a re-plan's copied ones too). Each now has a test that fails under that mutation (`tests/lib/dispatch-load-state.spec.ts`, `tests/lib/plan-lifecycle.spec.ts`, `tests/lib/driver-pack.spec.ts`, `tests/integration/plan-lifecycle-db.spec.ts`).
- **Second review of the simplified driver rules, fixed before merge** (one medium, four low; no change to the rules or the code, only tests and docs):
  - *Frozen loads were tested only as LOCKED* (medium): pass 0 takes every status but PLANNED, but a change to LOCKED alone passed every test, and a mid-day re-plan could then give a driver a trip over that driver's loading, dispatched or completed load, without a note. `tests/lib/dispatch-load-state.spec.ts` now runs each frozen status in both truck orders (the filled-in trip moved onto it loses the driver with a `CLASH` note naming the frozen load; candidate 2 takes the driver of the truck's frozen Load 1 for its new Load 2), and the fake-database re-plan job in `tests/lib/plan-lifecycle.spec.ts` runs with each frozen status.
  - *Two conditions of candidate 2 had no test* (low): the driver of the truck's nearest trip must be active (a frozen load can carry a driver deactivated since), and a frozen load without a driver is not the nearest trip "that already has a driver". Both are in the unit tests now.
  - *No test checked the truck code of a truck the new plan leaves out* (low): `applyScenario` looks up the trucks of the evidence loads too, so a `TRIP_GONE` note (a **MIN_TRUCKS** option) or a `CLASH` note naming a frozen load names that truck by its code. The fake-database trucks now have codes that differ from their ids (T02 for `T2`), so the notes (returned and stored) and the yellow text must carry the code; the integration test checks the code of the `TRIP_GONE` truck.
  - *"No driver" could not end a note on a trip with no driver* (low): the guide and [3.7](#37-step-5-reviewing-the-plan) / [3.9](#39-step-7-a-driver-per-load) said it could. The Driver list sends nothing for the "No driver" already selected, and a re-sent `null` changes nothing. Within the rules (a note ends when the dispatcher sets that trip's driver), the docs now say such a note ends when a driver is picked for the trip; `tests/lib/plan-lifecycle.spec.ts` pins it.
  - *The guide promised a re-plan message with the note count* (low): the dispatch screens show a job's message only while it runs or after it failed. The guide now points to the yellow notes after a re-plan; the "Use instead" toast still counts them.

  The first three were checked by mutation: the new tests fail when pass 0 takes LOCKED loads only, when candidate 2 skips the active check or counts a frozen load without a driver, and when the evidence trucks are left out of the truck lookup. `tests/lib/repo-guards.spec.ts` keeps the two withdrawn promises out of the docs.
