...
| `scripts/bench_dispatch.py`, `scripts/bench_replan.py`, `scripts/bench_day1.py` | Local benchmarks: synthetic dispatch day / re-plans around locked loads, two versions compared (PR5) / legacy day 1 |
...
- **Service minutes** (since PR5, [3.12](#312-settings-that-change-the-plan) "Default service time"): the customer's confirmed value (`serviceTimeConfirmed`), else the type profile's `serviceTimeMin` if set, else the tenant `defaultServiceTimeMin`. An unconfirmed stored `avgServiceTimeMin` is not used; migration `20260928090100_keep_imported_service_times` confirmed the times earlier imports had stored, and the pre-deploy count is in [5.12](#512-how-to-verify-a-deploy) check 11. The customer page shows the value in use and its source (`describeServiceTime`).
...
- **Bounds.** Every field is validated against `lib/planner-bounds.ts`, a copy of `packages/shared-types/src/planner-bounds.json`; each range lies inside the optimizer's Pydantic contract (`apps/solver/dispatch_models.py`). Tests check both sides against the JSON (`tests/lib/tenant-settings.spec.ts`, `apps/solver/tests/test_dispatch.py`). Overtime after must be at most the shift maximum, checked on the merged settings when a save changes the overtime threshold or the shift maximum (`overtimeSaveProblem`, the route and the form alike). A stored threshold after a lowered shift maximum is a planner warning and an amber note on the page; it no longer blocks saving the company name or another field (PR5 review).
...
**Default service time (PR5).** An unloading time a planner confirmed (customer screen, or an imported column) wins; otherwise the customer type's; otherwise the company default from Settings. Before PR5 an unconfirmed customer kept its column default (10 min) and the Settings default changed nothing for normal customers. A new customer created with an explicit unloading time counts as confirmed. Customer imports before PR2 stored the file's time without confirming it; migration `20260928090100_keep_imported_service_times` marks those times confirmed (only where the type has no time of its own and the value is not the 10 min column default; one audit row each), so they are not replaced by the Settings default. The customer page shows the unloading time in use and where it comes from.
...
| `apps/solver/scripts/bench_replan.py` | Re-plan benchmark around locked loads, two solver versions on the same requests (`state`, `run [--root <other checkout>/apps/solver]`, `compare`; see its docstring) | [`OPTIMIZER_BENCHMARK.md`](./OPTIMIZER_BENCHMARK.md) section 9.2 |
...
- **Costs (PR5, review F17; owner decision: the driver is paid for the WHOLE truck day).** One pure module, `apps/solver/costing.py` (`truck_day_costs`, policy `TRUCK_DAY_SPAN`), prices every truck day; `_build_scenario` reports it and `load_repack.score` compares candidates with it, so what the plan reports is what the optimizer minimized:
  ...
  - `operating_cost` = the sum of the loads. Each load's `total_cost` is its exact money rounded once to 0.001 OMR, and its six parts are rounded so that they add up to it exactly (`costing.round_parts`, largest remainder: each part within one baisa of its exact value, a zero part stays zero). The loads therefore add up to the exact truck-day money within 0.0005 OMR per load; a larger gap is logged and shown as a "Cost check" warning, never a failed optimization (PR5 review: the earlier check was tighter than the per-part rounding and failed 1-3 load re-plans with a 500). `truck_days[]` repeats the sum per truck;
...
- `apps/solver/scripts/bench_replan.py` compares two solver versions on re-plans around locked loads (PR5; run it for any change to how locked loads are planned).
...
| `20260928090100_keep_imported_service_times` | stabilization PR5 (review fixes) | **Data only, idempotent** (no schema change): marks `Customer.serviceTimeConfirmed = true` where it was false, `avgServiceTimeMin` is above 0 and not the 10 min column default, and the customer type has no `serviceTimeMin` - the unloading times earlier imports stored without confirming them, which PR5's rule would otherwise replace by the Settings default. One `UPDATE Customer` audit row per changed customer (`afterJson.by` names the migration), so the rows can be found and reverted. Old code reads the flag the same way, so an app rollback is safe |
...
11. **After PR5 (costs, routing, settings, audit).**
    - `_prisma_migrations` lists `20260928090000_plan_load_cost_json` and `20260928090100_keep_imported_service_times`, and `PlanLoad.costJson` exists.
    - Before the deploy (owner, read-only), unloading times. Every query counts customers whose type has no unloading time of its own (no type, or a type whose profile has none - not only untyped customers):
      - kept by the migration (it marks them confirmed): `SELECT count(*) FROM "Customer" c WHERE NOT c."serviceTimeConfirmed" AND c."avgServiceTimeMin" > 0 AND c."avgServiceTimeMin" <> 10 AND NOT EXISTS (SELECT 1 FROM "CustomerTypeProfile" p WHERE p."tenantId" = c."tenantId" AND p."customerType" = c."customerType" AND p."serviceTimeMin" IS NOT NULL);`
      - switching to the Settings default (stored 10 min, the column default, never confirmed; a change only where the default is not 10): `SELECT count(*) FROM "Customer" c JOIN "TenantConfig" t ON t."tenantId" = c."tenantId" WHERE NOT c."serviceTimeConfirmed" AND c."avgServiceTimeMin" = 10 AND t."defaultServiceTimeMin" <> 10 AND NOT EXISTS (SELECT 1 FROM "CustomerTypeProfile" p WHERE p."tenantId" = c."tenantId" AND p."customerType" = c."customerType" AND p."serviceTimeMin" IS NOT NULL);`
      - If the second count is above 0 and some of those customers really need 10 min, set their unloading time in the customer details on Daily dispatch (or re-import the customer file with the column) after the deploy; the customer page shows the time in use and its source. After the deploy, `SELECT count(*) FROM "AuditLog" WHERE "afterJson"->>'by' = 'migration 20260928090100_keep_imported_service_times';` equals the first count.
    ...
    - A re-planned day's dashboard figures include its carried locked loads. A late-order re-plan (typically one or two new loads) ends READY, and no plan shows a "Cost check" warning.
...
- **Review of PR5, fixed before merge** (all confirmed findings; each has a test):
  - *Cost check failed small optimizations with a 500* (high): each load's six parts were rounded to 0.001 OMR one by one, so a load drifted up to 0.003 OMR from its exact money, but `_build_scenario` allowed 0.001 per load and raised `CostError`, which `/optimize-dispatch` answered with 500 - deterministically, so a retry failed too. One- to three-load optimizations failed often (the reviewer: 5 of 40 late-order re-plans around a locked load), the core late-order case. Each load's total is now its exact money rounded once and its parts add up to it exactly (`costing.round_parts`), the check allows 0.0005 OMR per load, and a mismatch is logged and shown as a "Cost check" warning instead of failing the plan (`CostError` is gone) ([4.10](#410-extraction-and-reported-costs)). Tests: `test_costing.py` (the reviewer's one-load day, 16 random 1- and 2-load days fresh and around a locked load - two of them failed under the old rule -, 5,000 random part sets, a forced mismatch returns the plan with the warning); integration `dispatch-costs.spec.ts` checks the re-plan ends READY.
  - *Settings save refused over a stored overtime threshold* (low): a tenant whose shift maximum was lowered below the (then not editable) overtime threshold could not save even the company name. The rule is now checked only when a save changes one of the two, in the route and the form (`overtimeSaveProblem`); the page shows an amber note otherwise ([3.12](#312-settings-that-change-the-plan)).
  - *Handbook 3.4 still stated the old service-time rule* (low): corrected, with links.
  - *`distance_quality` MIXED after dropping the only off-road stop* (low): `_submatrix` now derives the quality (and `distance_is_estimated`) again from the kept legs, so a plan whose every leg is a road leg says ROAD (post-deploy check 11).
  - *Release benchmark not run, re-plans unmeasured* (low): measured, [`OPTIMIZER_BENCHMARK.md`](./OPTIMIZER_BENCHMARK.md) section 9. `bench_dispatch.py 300`: the same RECOMMENDED plan on PR4 and PR5 (12 trucks, 24 loads, 1,806.5 km, all served; the cost +1.6% only from the paid turnarounds). New `scripts/bench_replan.py` (two shapes of 8 days around locked loads, one cost model for both versions): nothing unserved either way; late-order re-plans give the same plan on 7 of 8 days and one day +24 OMR (one more truck, traced to the search's start bound for trucks with locked loads); with half the fleet locked PR5 is 0.6% cheaper with 2.8% fewer km. Over the 16 days the versions are within 0.8%, so the owner's whole-day rule stays in the search.
  - *No "*" on the Operating cost KPI of plans saved before PR5* (low): the plan screen and the workbook read the cost basis through one helper (`summaryCostBasis`: a summary without `costBasis` was costed the earlier way).
  - *Driver-rate contract comment on the wrong field* (low): the whole-truck-day JSDoc now documents `driver_cost_per_hour` in `packages/shared-types`.
  - *Unconfirmed imported service times replaced by the Settings default* (low): migration `20260928090100_keep_imported_service_times` keeps them; the pre-deploy count is widened to every customer whose type has no unloading time, with a remediation step ([5.12](#512-how-to-verify-a-deploy) check 11); the customer page shows the time in use and its source.
  - *Two "Large day" warnings above 350 stops* (low): the web's is dropped; the optimizer's names the limit.
  - *Workbook stated the new cost rules for plans costed the earlier way* (low): `planRules` (CURRENT, EARLIER, MIXED; from the option in use's cost version and the loads' breakdowns) words the ASSUMPTIONS rows (driver cost, overtime, road time factor, default service time), the NOTES and the TRUCK DAYS title by the rules the plan was made with.
- **Still open** (not in PR5): a `CustomerTypeProfile` editor; dropping the deprecated `TenantConfig` columns (expand-then-contract); retiring the solver's PyVRP `/optimize`; loads locked out of order keep their planned cost share (documented, flagged on TRUCK DAYS); re-measure re-plans on NMWC's real days with locked loads, and if the one worse late-order case recurs, add a search without the start bound as an extra candidate (OPTIMIZER_BENCHMARK 9.2). A 150-stop synthetic day on PR5 and on PR4 gave the same plan (8 trucks, 9 loads, 805.3 km, 0 unserved); the reported cost rose 478.5 -> 479.8 OMR because depot turnaround and waiting are now paid. The 300-stop release benchmark and the re-plan comparison: [`OPTIMIZER_BENCHMARK.md`](./OPTIMIZER_BENCHMARK.md) section 9.
