...
| `tests/test_dispatch.py` (47 tests), `tests/test_repack.py` (29), `tests/test_feasibility.py` (19, PR4), `tests/test_solver.py` (27, legacy), `tests/conftest.py` | Pytest. Uses Haversine or a mocked OSRM (`httpx.MockTransport`), so no network is needed. A few tests are parametrised; PR #29 reports 104 collected tests, PR4 144 |
...
| Plan options table | every `ScenarioResult` of the run: trucks, loads, km, cost, preferred-window penalty, unserved, **Timing checked** (PR4: the optimizer's own check of the option - Yes, "No - N rule(s) broken", "Not verified", or "not checked" for options from before the check), solver time and status; "In use" or "Use instead" (an option whose times break a rule can be used for review, but its trucks cannot be dispatched) |
| Red "Times not verified" box (PR4) | `detail.feasibility`: the blocking violations per truck (up to 8, all in the Excel SUMMARY) and a **Re-plan** button. Amber lines for warnings (cases with no weight, problems on loads already out). Each load row shows "Times not verified" when its truck-day fails, and Lock / Loading / Dispatch are disabled with the reason while the gate is on |
| "Changed after planning" (PR4) | `DetailStop.masterChanged` / `DetailLoad.masterChanged`: a pin moved more than 50 m, receiving hours, name or address changed in the customer master, or the truck's capacity changed, since the stop was planned. The plan keeps what it was planned with; the note is shown under the stop and as a badge on the load, and a warning says what to do (re-plan a PLANNED load; unlock and re-plan a locked one). "current customer data" marks a stop planned before snapshots existed |
...
**The feasibility gate (PR4, review F04).** LOCK, LOADING and DISPATCH (forward moves only: `isGatedMove`) run `timingGate` in `changeStatusTx`, under the plan's row lock. It recomputes the timetable check of the version from the plan's own facts (`loadFeasibilityInput` + `checkPlanFeasibility` in `lib/dispatch/feasibility.ts`; the stored `RunPlan.feasibilityJson` is only compared by its `inputHash`) and refuses the move with **409 `TIMES_NOT_VERIFIED`** (`{ error, code, truckId, truckCode, status, violations[] }`, message "Truck T01: the timetable is not verified … Re-plan to get a timetable that keeps every rule") when the load's **truck-day** is not OK. Checked per truck-day over every load of the version, frozen copies included, never against today's master data:

- cases per load (from its stops) against the truck it was planned on (`truckSnapshotJson`); **physical kg** (a split portion's own kg, else the order's) against that payload (review F01). Cases with no weight are `KG_UNKNOWN`, a **warning** only: the dispatcher accepted them at optimize ("treated as 0 kg", review F02);
- each service start inside the receiving window the stop was planned with (`stopSnapshotJson`), and the stored `hardWindowOk`;
...
| `LOAD_LOCKED`, `LOAD_PLANNED`, `LOAD_LOADING`, `LOAD_DISPATCHED`, `LOAD_COMPLETED` | PlanLoad | `changeStatusTx`. A gated move (PR4) records `timing` (status, ok, gate, and the overridden violations under `FEASIBILITY_GATE=warn`) |
...
| `RouteAssignment` | one order (or split portion) at one stop of one load | unique (`runId`, `truckId`, `loadNo`, `sequenceInTruck`, `orderInStop`); `loadId`, `etaMin`, `serviceStartMin`, `departureMin`, `waitMin`, `cumulativeKm`, `hardWindowOk`, `prefWindowOk`, `portionCases`, `portionWeightKg`, `portionLinesJson`, `stopSnapshotJson` (PR4: the pin, hours, service time and priority sent to the optimizer, plus the customer's code, name, type, address and access notes when applied; null before PR4) |
...
**Expected counts at `f2f4099`** (from PR bodies): solver 104, web unit 344, integration 74. After PR1: solver 117, web unit 522 (33 files). After PR4 (stacked on PR3): solver 144, web unit 704 (44 files, incl. tenant isolation).
...
| `driver-pack.spec.ts` | `lib/dispatch/plan-detail.ts`, `driver-pack.tsx` | driver-sheet PDF model and rendering, WhatsApp links, driver clashes; (PR4) "Location updated after planning" and TIMES NOT VERIFIED |
...
| `plan-snapshots.spec.ts` (PR4) | `lib/dispatch/snapshots.ts`, `planInputsOf`, `masterChangedNotes`, `jobMessage`, startup checks | what counts as changed after planning, inputs without the OSRM address, notes, job message, `FEASIBILITY_GATE` warning |
...
| **A plan shows what it was planned with** (PR4, review F08) | `ScenarioDetails.inputs`, `RouteAssignment.stopSnapshotJson` and `PlanLoad.truckSnapshotJson` written by `applyScenario`, copied by `createNextVersion` (`copyRowData`, NULL-safe); `getPlanDetail`, `load-geometry`, the driver sheets and the workbook read the snapshots and only flag live changes (`masterChanged`) | `plan-feasibility-gate.spec.ts`, `plan-snapshots.spec.ts`, integration `dispatch-mvp.spec.ts` (27-29) | A pin, window or truck correction silently changing a locked, dispatched or superseded load's sheet; the ASSUMPTIONS sheet built from today's settings under a "built with" title; a re-plan of pre-snapshot rows throwing on a Json null |
