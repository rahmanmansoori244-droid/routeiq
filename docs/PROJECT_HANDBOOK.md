...
- It picks a driver for every new load with `planReplanDrivers` ([3.9](#39-step-7-a-driver-per-load)) before anything is deleted, and lists the trips whose driver changed.
...
- It sets `chosenScenarioId` and the status by the loads: `DISPATCHED` (with `finalizedAt`) when every load of the version is already out, otherwise `READY` (PR3: before, always READY). The write is conditional (never over a superseded row). It then calls `refreshPlanFacts` with the driver changes and writes audit `SCENARIO_CHOSEN` (with `driverChanges`: truck, trip, from, to, reason, when any). It returns `{ driverChanges }`: `chooseScenario` passes them on (choose-scenario answers `driversChanged`, the count the screen's toast names), and the job's message adds "N trip(s) with another driver". This happens on every optimize, not only when the dispatcher switches options.
...
- `summaryJson` = `computeSummary` (`apps/web/lib/dispatch/summary.ts`): orders served, partial and unserved; cases; service % per P1 to P5; trucks, loads, km, hours, utilisation, fuel, cost; revenue and margin served (null unless every order has a value); late orders; unserved by reason; loads by status; provider and solver info; and `driverChanges` (fourth review of PR3), the trips whose driver the applied plan changed (`DriverChangeNote`: truck, trip, times, from and to with their names, reason, the overlapping load). `applyScenario` passes the new list (the key is left out when it is empty); a refresh after a load change keeps the list already stored.
...
| Yellow warnings | request and solver warnings from the chosen `ScenarioDetails`; "Driver changed by this plan: ..." for each trip of `summaryJson.driverChanges` that still has the driver the plan gave it (`driverChangeWarnings` in `driver-links.ts`) |
...
- Otherwise it runs `applyScenario` on the same version. No new version is created, and `planReplanDrivers` keeps the drivers; the toast says how many trips got another driver.
...
- **Per load.** The driver picker calls `PATCH /api/runs/:id/loads/:loadId { driverId }` (`null` = no driver), which runs `updateLoad` then `setDriverTx`.
  ...
  - The load is marked as the dispatcher's choice: `driverSetById` (the user) and `driverSetAt` (fourth review of PR3, migration `20260926110000_plan_load_driver_set_by`). "No driver" clears both. `createNextVersion`'s copies keep them. A driver set before that migration has no marker and counts as filled in by RouteIQ (no backfill: the `LOAD_DRIVER_SET` rows name the original load only, not its copies in later versions).
...
- **After every optimize, re-plan or "Use instead".** `applyScenario` calls `planReplanDrivers(newLoads, now, parent, kept, usable)` (`load-state.ts`; `assignReplanDrivers` returns its drivers only). `now` = this version's loads: its frozen loads, and loads whose driver was decided on this version. A PLANNED copy a re-plan carried from the parent (copy-forward) untouched - the parent load's driver and marker unchanged - does **not** count (`ownDriverEvidence`): nobody chose that driver on this version, so it is parent evidence (review of PR3: counting those copies gave the parent's drivers without the clash check). A trip's evidence is the same truck and trip before this plan: in `now`, else in the parent.
  ...
  - **Every other driver is RouteIQ's pick**, from the strongest evidence first:
    1. the driver of the same truck and trip in `now`;
...
| `SCENARIO_CHOSEN` | RunPlan | `applyScenario` (every optimize and every "Use instead"); `driverChanges` lists the trips whose driver changed (truck, trip, from, to, reason) |
...
| `LOAD_DRIVER_SET` | PlanLoad | `setDriverTx` (the load is also marked: `driverSetById`, `driverSetAt`) |
...
| `dispatch-load-state.spec.ts` | `lib/dispatch/load-state.ts` | load status graph, load order per truck, frozen/on-road sets, driver carry-over across re-plans (hand-set drivers kept, the trip that moved loses a filled-in driver in every step, the reported driver changes) |
