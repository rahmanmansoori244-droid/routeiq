...
- **After every optimize, re-plan or "Use instead".** `applyScenario` calls `assignReplanDrivers(newLoads, now, parent, kept, usable)` (`load-state.ts`). For each new load it takes the strongest evidence first:
  ...
  No step, step 1 included, puts a driver on a load that overlaps one of that driver's kept (locked, loading or dispatched) loads on another truck: a kept load cannot move, so that is always a double booking (`timesClash` against `kept`). Such a load goes to the next step, or stays without a driver. Steps 2 to 4 are guesses and also never overlap loads already given out in this plan. Only active drivers of the tenant count. "No driver" is not carried over. Second review of PR3: a PLANNED load the job had just created (not a copy) was step-1 evidence without any check, so "Use instead" re-timing its trip onto the hours of the driver's LOCKED load gave that driver two sheets.
- **Clashes.** A clash the dispatcher sets by hand is allowed but shown (`driverClashes`, `driverClashNotes`). The one clash a re-plan or "Use instead" can keep: two new planned loads whose trips now overlap and that both keep this version's driver for the trip (step 1); it shows the same warning. A hand-set clash with a kept load is not carried into the next plan: that load gets the next source or no driver.
