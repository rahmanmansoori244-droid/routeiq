...
| | `runs/[id]/optimize` | POST PLANNER (30/h/tenant) | Start a background optimize → 202 + `runJobId` |
...
sequenceDiagram
  ...
  SO->>SO: buildDispatchRequest in plan-service.ts
  SO->>DB: create RunJob QUEUED and set RunPlan OPTIMIZING in one transaction
  ...
  JOB->>DB: transaction - persistDispatchResult, RunPlan READY, applyScenario RECOMMENDED, RunJob SUCCEEDED
  loop every 2.5 to 3 s while OPTIMIZING
    UI->>RH: GET /api/dispatch/day or /api/runs/:id/plan
  end
```

- **One job per plan.** `trackInflight(runId, …)` in `lib/jobs/optimize-job.ts` keeps an in-memory `Map` on `globalThis.__routeiqInflight`, shared with the instrumentation bundle. A second start for the same run returns the active job.
- **On failure.** `failJob()` sets RunJob FAILED with `errorJson`, sets RunPlan FAILED, and writes an `OPTIMIZE_FAILED` audit row.
- **Why exactly one replica.** The in-flight map, the rate-limit buckets and the geometry LRU all live in process memory. `README.md` and the header of `lib/jobs/optimize-job.ts` therefore require **exactly one web replica**.
...
3. The route adds an error when a batch with the same `fileHash` (= `contentHash`, so for the same dates) was already CONFIRMED for this depot. It creates an `UploadBatch` with status `PARSED` (has errors) or `VALIDATED` (no errors), stores the full validation result (including every resolved line) in `validationJson`, and writes audit `CREATE UploadBatch`. The `deliveryDate` form field must be a real date (`isRealIsoDate`).
...
- **Re-check inside the transaction** (review F05). The transaction first takes `lockIntake` (`pg_advisory_xact_lock(hashtextextended('intake:' || tenantId, 0))`, the same lock as the late-order route and batch delete), then `SELECT ... FOR UPDATE` on the batch row, then `revalidateIntake` (`intake-server.ts`). Each conflict answers 409 with a `code`, saves nothing and leaves the batch `VALIDATED`:
  ...
  - `DUPLICATE_FILE`: another batch with the same `fileHash` and depot is CONFIRMED;
...
- **Writes.** `confirmIntake`:
  ...
  - creates one `Order` per customer branch and delivery date, with one `OrderLine` per SKU line (sales order number, product, cases, kg, value, margin, source row, notes). Line kg = the file kg of the rows that had one + `weightMissingCases` × the product's case weight; 0 when neither is known. Order totals are summed from its lines;
...
- one issue card per customer branch, with effective priority, window text, issues and a `blocking` flag, blocking ones first. A deactivated customer shows only `CUSTOMER_INACTIVE` (blocking; its open orders are left unserved at optimize);
- weights per order **line**, not per product (`lineWeightStatus` in `apps/web/lib/dispatch/weights.ts`), for open lines that are not on a frozen load of a live version: `productsWithoutWeight` (0 kg and no case weight on the product: counted as 0 kg) and `weightsToApply` (0 kg, but the product has a case weight now: applied at the next optimize), each with lines and cases per product;
...
1. the plan exists;
2. it is not `SUPERSEDED`;
3. it is not a legacy plan, else 409 `LEGACY_PLAN`. `isLegacyPlan` is true when the plan has `RouteAssignment` rows without a `loadId`, or its chosen scenario has no dispatch `scope` and `loads`;
...
It then runs `resolveOrderWeights` in its own transaction, calls `buildDispatchRequest`, and refuses missing locations and unknown weights (unless allowed, `gate` in `start-optimize.ts`), a day with no orders to plan (400) and a depot with no active trucks (400).

**`resolveOrderWeights(tx, tenantId, runId, userId)`** (`plan-service.ts`, pure part `resolveOrderLineWeights` in `weights.ts`). A line weight of 0 means unknown. For the day's orders that are not DISPATCHED / DELIVERED and have no assignment on a frozen load of a live (not SUPERSEDED) version, every 0-kg line whose product now has a case weight gets cases × that weight, and the order total is summed again. Orders whose kg lives on the order only (older orders: lines at 0 kg, order total above 0) are left as they are. One audit row `ORDER_WEIGHTS_RESOLVED` (entity RunPlan) lists each line and order before and after. It also runs in `replan` before the probe; resolved weights may change orders on superseded PLANNED loads (their stops then show the new kg), never frozen ones. Finally, in one transaction it creates the `RunJob` (`attemptNo` + 1, `QUEUED`, `requestJson`) and sets `RunPlan.status = OPTIMIZING` and `currentJobId`. It writes audit `OPTIMIZE_STARTED`, calls `scheduleDispatchOptimize` without waiting, and answers 202.
...
| Weights | Per line: its own kg (`weightKg / cases`); for an order whose kg lives on the order only, the order kg per case; for the open rest of a partly frozen order, a 0-kg line takes the product's case weight in memory. Lines still at 0 kg are counted in `unknownWeights` (the `WEIGHT_REQUIRED` gate) |
...
1. sets the `RunJob` to `RUNNING`, 20 %;
2. calls `callDispatchSolver` (`apps/web/lib/solver-client.ts`): POST to `SOLVER_URL` + `/optimize-dispatch` with header `X-Solver-Token`, up to 600 s ([2.6](#26-runtime-architecture));
3. stores `responseJson`, 80 % ("Saving plan");
4. in one transaction (timeout 120 s) runs `persistDispatchResult`, sets `RunPlan.status = READY`, runs `applyScenario` with the RECOMMENDED scenario, and marks the `RunJob` `SUCCEEDED` at 100 % with a message such as "N loads on M trucks, K stop(s) unserved";
5. writes audit `OPTIMIZE_SUCCEEDED` with per-scenario figures.

Any error goes to `failJob`; lost jobs are reaped by the janitor ([5.9](#59-job-failures-and-the-janitor)).

**`persistDispatchResult(tx, tenantId, runId, built, resp)`** deletes the run's old `ScenarioResult` rows and writes one per scenario with its totals and `detailsJson`. `ScenarioDetails` is the solver scenario plus engine, distance provider, estimated flag, warnings and `scope`. Each scenario gets its `UnservedOrder` rows: the pre-drops plus the solver's unserved stops, with split parts merged per order and reason into `portionLinesJson`.

**`applyScenario(tx, tenantId, runId, scenarioId, userId)`** turns a scenario into loads:

- It refuses a SUPERSEDED run and a scenario without dispatch details (legacy).
...
sequenceDiagram
    ...
    SO->>PS: resolveOrderWeights
    PS->>DB: 0-kg open lines get the product case weight, AuditLog ORDER_WEIGHTS_RESOLVED
    SO->>PS: buildDispatchRequest
    PS->>DB: read orders, lines, customers, type profiles, trucks, frozen loads, TenantConfig
    PS-->>SO: request, preDrops, scope, blocking, warnings, unknownWeights
...
| Truck loads table | one row per `PlanLoad`: truck · load, driver picker with PDF and WhatsApp links, status, depart to return, stops, cases against capacity, utilisation, km, time, fuel, cost, action buttons. Expanding a row shows the loading manifest (SKU totals from `aggregateSkus`) and the delivery route (ETA, window, cases, SKUs, sales orders, km, "Part k of n" for split deliveries) |
...
- It is refused while the plan is OPTIMIZING or SUPERSEDED, and when any load was locked in this version (a frozen load whose `carriedFromLoadId` is null).
...
Around every change, `lockOpenRun` takes a row lock on the `RunPlan` and refuses SUPERSEDED and OPTIMIZING plans. After the change:
...
- the plan becomes `DISPATCHED` (and `finalizedAt` is set) when every load is DISPATCHED or COMPLETED, otherwise `READY`;
...
- **After every optimize, re-plan or "Use instead".** `applyScenario` calls `assignReplanDrivers(newLoads, now, parent, kept, usable)` (`load-state.ts`). For each new load it takes the strongest evidence first:
  1. the driver of the same truck and trip in this version;
...
- use "Late order" on the plan (`late-order-dialog.tsx`). It calls `POST /api/dispatch/late-order` (`apps/web/app/api/dispatch/late-order/route.ts`) with customer code, optional branch and name, priority, reason (at least 3 characters) and lines (product code, cases, sales order). In one transaction the route:
  - takes `lockIntake` (the same per-tenant lock as confirm and batch delete);
  ...
  - refuses (409 `DUPLICATE_LINES`) a line whose sales order + product is already confirmed for that customer and date (`IntakeLineKey`), and (400) the same product twice on one sales order;
  - creates one `Order` with its lines: `status VALIDATED`, `isLate` = after the cutoff or a plan is already applied, `lateReason`, `lateRecordedById`. `lateReason` and `lateRecordedById` are stored even when `isLate` is false. Line kg = cases × the product's case weight (0 = unknown). One `IntakeLineKey` per line with a sales order.
...
3. otherwise it runs `resolveOrderWeights`, then probes `buildDispatchRequest(parent)` and answers 409 `LOCATION_REQUIRED` or `WEIGHT_REQUIRED` (same `gate` as optimize, with `allowMissingLocations` / `allowMissingWeights`) before any version is created;
...
5. `createNextVersion` runs one transaction with `SELECT ... FOR UPDATE` on the parent and refuses a SUPERSEDED or OPTIMIZING parent. It:
   ...
   - copies every frozen `PlanLoad` (new id, `carriedFromLoadId` = old id, same status, driver and times) with its `RouteAssignment` rows;
   - marks the parent `SUPERSEDED` with `supersededAt`;
   - writes audit `PLAN_VERSION_CREATED`;
6. `startDispatchOptimize(child)`. In `buildDispatchRequest(child)`, the copied frozen loads keep their orders and cases, each truck's `frozen_trips` are fixed, and `previous_truck_id` is sent only when `usesPlanContinuity` is true. The background job then runs exactly as in [3.6](#36-step-4-optimize), on the child, and `refreshPlanFacts` writes the change summary against the parent.
...
Older versions stay readable at `/t/[slug]/dispatch/plan/[id]`. On a SUPERSEDED version every editing control is hidden, and the server refuses changes anyway (`lockOpenRun`, `applyScenario`, `startDispatchOptimize`, choose-scenario).
...
sequenceDiagram
    ...
    D->>UI: Re-plan now
    UI->>RP: reason LATE_ORDER
    RP->>SO: replan
    SO->>PS: isLegacyPlan on the parent
    alt parent has no applied scenario
        SO->>SO: startDispatchOptimize on the same version
    else parent is applied
        SO->>PS: buildDispatchRequest on the parent as a probe for missing locations
        SO->>PS: pendingLateOrderIds - a REOPTIMIZE with late orders waiting becomes LATE_ORDER
        SO->>PS: createNextVersion
        PS->>DB: lock parent FOR UPDATE, create child version N+1 as DRAFT with parentRunId and reason
        PS->>DB: copy frozen PlanLoads and their RouteAssignments with carriedFromLoadId
        PS->>DB: parent SUPERSEDED, AuditLog PLAN_VERSION_CREATED
        SO->>SO: startDispatchOptimize on the child
    ...
    Note over UI,DB: the background job then runs as in the optimize diagram, on the child version
```
...
| `ORDER_WEIGHTS_RESOLVED` | RunPlan | `resolveOrderWeights` (before every optimize and re-plan probe): each line and order kg before and after |
...
| Value | Set by | Next |
|---|---|---|
| DRAFT | schema default: `getOrCreatePlan` (version 1), `createNextVersion` (child), legacy `POST /api/runs` | OPTIMIZING |
| OPTIMIZING | `startDispatchOptimize`, together with the new `RunJob` | READY, FAILED |
| READY | `dispatch-job.ts` after saving; `applyScenario`; `changeStatusTx` while not every load is out | DISPATCHED, SUPERSEDED |
| FAILED | `failJob` in `dispatch-job.ts`; `reapStuckJobs` (only from OPTIMIZING) | OPTIMIZING (optimizing a FAILED plan again is allowed), SUPERSEDED |
| DISPATCHED | `changeStatusTx` when every load is DISPATCHED or COMPLETED (sets `finalizedAt`); legacy `POST /api/runs/:id/dispatch` for runs without loads | SUPERSEDED (a re-plan is not blocked); legacy `/unlock` back to READY for runs without loads |
| SUPERSEDED | `createNextVersion` on the parent (sets `supersededAt`) | final, read-only |
| ARCHIVED | never written by any code; only excluded by `currentPlan` | none |

**`PlanLoad.status` (`LoadStatus`):** see [3.8](#38-step-6-load-states). Created as PLANNED by `applyScenario`, or copied with its current status by `createNextVersion`.
...
| `OrderLine` | one SKU line | `productId`, `cases`, `salesOrderNo`, `weightKg` (0 = unknown, resolved at optimize once the product has a weight), `salesValue`, `marginValue`, `sourceRow`, `notes`; cascades with its order |
| `IntakeLineKey` | identity of a confirmed sales-order line | unique (`tenantId`, `deliveryDate`, `salesOrderNorm` (trimmed, upper-case), `customerId`, `productId`); `orderLineId` unique, cascades with the line; `uploadBatchId` (null for a late order). Written by `confirmIntake` and the late-order route; backfilled for existing lines by migration `20260926090000_intake_line_keys` |
...
| `PlanLoad` | one physical truck trip | unique (`runId`, `truckId`, `loadNo`); `status`, `driverId`, `departMin`, `returnMin`, `distanceKm`, `durationMin`, `cases`, `weightKg`, `utilizationPct`, `fuelLitres`, `fuelCost`, `operatingCost`, `returnLegKm`, `distanceIsEstimated`, `carriedFromLoadId`, `statusChangedAt`, `statusChangedById` |
...
**Delete behaviour.** Deleting a `RunPlan` cascades to its jobs, scenarios, loads and assignments. `RouteAssignment.orderId` is `ON DELETE RESTRICT` and `UnservedOrder.orderId` is `ON DELETE NO ACTION` (since migration `20260926090100_unserved_order_fk_no_action`; it was CASCADE), so an order that is on any plan option, planned or unserved, cannot be deleted. Deleting a tenant therefore removes its plan data first (as `cleanupTenant` in the integration helpers does).
...
| POST | `/api/dispatch/plan` | PLANNER | `getOrCreatePlan`, `startDispatchOptimize` | `DispatchClient.optimize` when no plan is applied |
| POST | `/api/runs/:id/replan` | PLANNER | `replan`, `createNextVersion`, `startDispatchOptimize` | `DispatchClient.optimize` when a plan is applied; `PlanView` Re-plan and late-order follow-up |
| POST | `/api/runs/:id/optimize` | PLANNER, rate limited | `startDispatchOptimize` | legacy `run-detail.tsx` only |
| GET | `/api/runs/:id/plan` | any | `getPlanDetail` | `PlanView` |
| POST | `/api/runs/:id/choose-scenario` | PLANNER | `applyScenario` | `PlanView` "Use instead"; legacy `run-detail.tsx` |
| PATCH | `/api/runs/:id/loads/:loadId` | PLANNER (DISPATCHED and COMPLETED need SUPERVISOR) | `updateLoad` | `PlanView` status buttons, "Lock all loads", driver picker |
...
Environment variables used on this path: `SOLVER_URL`, `SOLVER_TOKEN`, `OSRM_URL` (solver side; the workbook assumptions report whether the web has one), `JANITOR_TOKEN`, `ROUTEIQ_DISABLE_JANITOR`.
...
| `20260926090000_intake_line_keys` | stabilization PR2 | New table `IntakeLineKey` (unique sales-order line identity, see [3.15](#315-data-model-used-by-the-dispatch-flow)), backfilled from every existing line with a sales order. Where the same line was already confirmed twice, the oldest line keeps the key (`ON CONFLICT DO NOTHING`) and the copies stay as they are. Additive; rollback is `DROP TABLE` |
| `20260926090100_unserved_order_fk_no_action` | stabilization PR2 | `UnservedOrder.orderId` foreign key from `ON DELETE CASCADE` to `NO ACTION` (added `NOT VALID`, then validated) |
...
- **Janitor.** `reapStuckJobs()` in `apps/web/lib/jobs/optimize-job.ts` together with `reapStaleShifts()` in `apps/web/lib/jobs/shift-janitor.ts`:
  ...
  - **Effect:** the job gets `errorJson.reason = 'STUCK'` and the message "No result after 15 minutes (the server restarted during the optimization). Optimize again." A plan still `OPTIMIZING` becomes `FAILED`, and an `OPTIMIZE_FAILED` audit row is written.
...
| 2 | `apps/web/lib/dispatch/start-optimize.ts` (`replan`, `startDispatchOptimize`) | Version creation and job start | Lead L13 in [7.4](#74-review-leads-found-while-writing-this-handbook) |
| 3 | `applyScenario` via `api/runs/[id]/choose-scenario/route.ts` and `lib/jobs/dispatch-job.ts` | Applies a scenario and ends by setting `status: 'READY'` | Lead L15 in [7.4](#74-review-leads-found-while-writing-this-handbook) |
...
- **F02**: 0 kg = unknown; `resolveOrderWeights` before optimize / re-plan (audited `ORDER_WEIGHTS_RESOLVED`); 409 `WEIGHT_REQUIRED` with an "optimize anyway (0 kg)" override and a persistent warning; line-based weight notes on the day; intake weight fixes (file 0 kg, partial merges, new SKUs, suspicious per-case figures).
